...
def load_redesign_module():
    ...
    spec = importlib.util.spec_from_file_location("daily_credit_redesign_23", path)
...
def load_vc22_module():
    path = ROOT / "scripts" / "22_vix_credit_state_machine_test.py"
    spec = importlib.util.spec_from_file_location("vix_credit_22", path)
    if spec is None or spec.loader is None:
        raise ImportError(f"Unable to load module from {path}")
    mod = importlib.util.module_from_spec(spec)
    sys.modules[spec.name] = mod
...
def main() -> None:
    ...
    spec = mod22.StrategySpec("VC_ANCHOR_EXIT_FINAL_SCOPE", "anchor", "final_scope")
    state = mod22.simulate_vix_credit(panel, spec)
    panel["vc_anchor_stress"] = state["stress_active"].astype(bool)
    panel["vc_anchor_cross_state"] = panel["final_regime_confirmed"].astype(str) + "_" + panel["vc_anchor_stress"].map({False: "NORMAL", True: "STRESS"})

    behavior = asset_behavior(panel, "vc_anchor_cross_state")
    behavior.to_csv(TABLE / "vc_anchor_cross_state_asset_behavior.csv", index=False)
    counts = panel.groupby("vc_anchor_cross_state").size().rename("n_days").reset_index()
    counts.to_csv(TABLE / "vc_anchor_cross_state_day_counts.csv", index=False)

    plot_heatmap(
        behavior,
        "vc_anchor_cross_state",
        "annualized_return",
        FIG / "vc_anchor_cross_state_asset_behavior_heatmap.png",
        "Asset Annualized Return by VC_ANCHOR_EXIT_FINAL_SCOPE Regime x Stress State",
        percent=True,
    )
    plot_heatmap(
        behavior,
        "vc_anchor_cross_state",
        "Sharpe",
        FIG / "vc_anchor_cross_state_asset_sharpe_heatmap.png",
        "Asset Sharpe Ratio by VC_ANCHOR_EXIT_FINAL_SCOPE Regime x Stress State",
        percent=False,
    )
    print("generated VC anchor cross-state asset behavior heatmaps")
    print(str(TABLE / "vc_anchor_cross_state_asset_behavior.csv"))
    print(str(FIG / "vc_anchor_cross_state_asset_behavior_heatmap.png"))
    print(str(FIG / "vc_anchor_cross_state_asset_sharpe_heatmap.png"))
