...
def load_redesign_module():
    ...
    spec = importlib.util.spec_from_file_location("daily_credit_redesign_31", path)
...
def build_variants() -> list[Variant]:
    return [
        Variant("VC_ANCHOR_ALL_INVERTED_STEEP_OFF", steep_vix_enabled=False, steep_credit_enabled=False),
        Variant("VC_ANCHOR_ALL_INVERTED_STEEP_VIX", steep_vix_enabled=True, steep_credit_enabled=False),
        Variant("VC_ANCHOR_ALL_INVERTED_STEEP_CREDIT", steep_vix_enabled=False, steep_credit_enabled=True),
        Variant("VC_ANCHOR_ALL_INVERTED_STEEP_ALL", steep_vix_enabled=True, steep_credit_enabled=True),
    ]
...
def credit_entry_cond(row: pd.Series, variant: Variant) -> bool:
    regime = row["refined_regime_confirmed"]
    enabled = regime in {"FLAT_LOW_RATE", "FLAT_HIGH_RATE", "INVERTED"} or (regime == "STEEP" and variant.steep_credit_enabled)
    if not enabled:
        return False
    return bool(
        (row["SPY_DD"] <= -0.05)
...
def perf_row(frame: pd.DataFrame, strategy: str, variant: Variant | None = None) -> dict[str, object]:
    ...
    row = {
        ...
        "steep_stress_days": int((frame["stress_active"] & frame["regime"].eq("STEEP")).sum()),
    ...
    if variant is not None:
        ...
        row["steep_credit_enabled"] = variant.steep_credit_enabled
    return row
...
def crisis_row(frame: pd.DataFrame, strategy: str, window: str, start: str, end: str | None) -> dict[str, object]:
    ...
    return {
        ...
        "steep_stress_days": int((sub["stress_active"] & sub["regime"].eq("STEEP")).sum()),
...
def main() -> None:
    ...
    perf_df.to_csv(OUT / "vc_anchor_steep_enable_performance.csv", index=False)
    crisis_df = pd.DataFrame(crisis_rows)
    crisis_df.to_csv(OUT / "vc_anchor_steep_enable_crisis.csv", index=False)
    ...
    challengers.to_csv(OUT / "vc_anchor_steep_enable_ranked.csv", index=False)

    names = [SPY_CASH_TIMING] + challengers["strategy"].tolist()
    plot_curves(frames, names, "nav", FIG / "vc_anchor_steep_enable_equity_curve.png")
    plot_curves(frames, names, "drawdown", FIG / "vc_anchor_steep_enable_drawdown_curve.png")
    ...
    print("\nSteep enable challengers:")
    print(challengers[["strategy", "CAGR", "Sharpe", "MaxDD", "Final Equity", "steep_stress_days", "inverted_stress_days", "composite_score"]].to_string(index=False))
